...
<pre>
<code>
    ./run.sh which_mapper [-c | --use-combiner] [--hdfs-root=root | input_directory output_directory]
...
*Note:* If you used the @--hdfs-root=root@ option with @setup.sh@, use the same option here. In this case, the @input_directory@ will be @root/input@ and the @output_directory@ will be @root/output@.

The only required argument is the one that specifies the kind of mapper to use:
...
h4. Running Hadoop Directly

The @run.sh@ script sets up the arguments to the application and the @HADOOP_CLASSPATH@. Here is a command line that would run Hadoop directly, if you don't want to use @run.sh@.

<pre>
<code>
    env HADOOP_CLASSPATH=target/scala_2.8.1/classes/:project/boot/scala-2.8.1/lib/scala-library.jar hadoop wordcount.WordCount no-buffer $HOME/word-count/input $HOME/word-count/output
</code>
</pre>

Where it the @no-buffer@ mapper is used and the @word-count@ HDFS directory is in @$HOME@. Notice that we have to set the @HADOOP_CLASSPATH@ to contain the compiled artifacts (as built by @sbt@) and the Scala runtime jar (using the location in the @sbt@ project).

Here is a variation of the command using two lines:

<pre>
<code>
    export HADOOP_CLASSPATH=target/scala_2.8.1/classes/:project/boot/scala-2.8.1/lib/scala-library.jar 
    hadoop wordcount.WordCount no-buffer $HOME/word-count/input $HOME/word-count/output
</code>
</pre>
...
<pre>
<code>
    hadoop dfs -cat $root/word-count/output-*/part-00000 | head -20
</code>
</pre>

Here @$root@ is either @$HOME@ or the the @--hdfs-root=root@ value.

The output will be thousands of lines. Hence, the @head -20@ limits the output to 20 lines. Note that you'll have to pick the output directory that corresponds to the kind of mapper used. The suffix after the @output-*@ will be one of @output-no-buffer@, @output-no-buffer-use-tokenizer@, @output-buffer@, @output-buffer-flush@.
...
# Implement other text processing algorithms.
